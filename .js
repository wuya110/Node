...
// // proxyIP 格式：ip、域名、ip:port、域名:port等,没填写port，默认使用443
let password = '12e2f86a-fb66-4ad6-8e83-b4655ea51c8f';  // 🟢 已修改为你指定的 UUID
let SSpath = '';
// // 路径验证，为空则使用UUID作为验证路径
...
// return {
                ...
                host: url.hostname,
                port: parseInt(url.port) ||
// (serverStr.startsWith('https://') ? 443 : 80),
                username: url.username ?
// decodeURIComponent(url.username) : '',
                password: url.password ?
// decodeURIComponent(url.password) : ''
...
// writer.releaseLock();
            ...
            //    throw new Error('Speedtest site 
// is blocked');
            //}
            if (addressType === 2) { 
...
// if (proxyConfig && (proxyConfig.type === 'socks5' || proxyConfig.type === 'http' || proxyConfig.type === 'https')) {
            shouldUseProxy = true;
// } else if (!proxyConfig) {
            proxyConfig = parsePryAddress(proxyIP) ||
// { type: 'direct', host: proxyIP, port: 443 };
        }
    } else {
        proxyConfig = parsePryAddress(proxyIP) ||
// { type: 'direct', host: proxyIP, port: 443 };
...
// }
    ...
    const forceProxyDomains = [
        'cloudflare.com',
        'openai.com', 'chatgpt.com', 'ai.com', // GPT 相关
...
// // 检查 host 是否包含上述关键词
    const isForceProxy = forceProxyDomains.some(domain => host.includes(domain));
...
// }
    ...
    // 如果满足强制代理条件，或者本身配置了 socks5/http 代理，则走 connecttoPry
    if (shouldUseProxy || isForceProxy) {
        try {
            await connecttoPry();
// } catch (err) {
            throw err;
...
// env.password || env.uuid || env.UUID || password;
            subPath = env.SUB_PATH || env.subpath || subPath;
            SSpath = env.SSPATH || env.sspath ||
// SSpath;
...
// } catch (e) {
                ...
                const customProxyIP = wsPathProxyIP ||
...
// // 🟢 删除了 ?ed%3D2560 并保留了 skip-cert-verify%3Dtrue
                        const ssNodeName = nodeName ? `${nodeName}-${ssHeader}` : `${ssHeader}`;
                        const encodedConfig = btoa(ssConfig);
                        return `${ssHeader}://${encodedConfig}@${host}:${port}?plugin=v2ray-plugin;mode%3Dwebsocket;host%3D${currentDomain};path%3D${validPath}/;tls;sni%3D${currentDomain};skip-cert-verify%3Dtrue;mux%3D0#${ssNodeName}`;
                    });
